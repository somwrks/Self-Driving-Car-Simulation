...
export default function Home() {
  ...
  const addTrafficCar = () => {
    if (!road || !carsRef.current) return;

    const randomLane = Math.floor(Math.random() * 3);
    // Calculate minY based on the best car's position from carsRef
    const bestCarY = Math.min(...carsRef.current.map((car) => car.y));
    const minY = bestCarY - window.innerHeight * 0.3; // Position new car ahead of best car
    ...
    setTraffic((prevTraffic) => {
      // Remove cars that are too far behind the best car
      const visibleCars = prevTraffic.filter(
        (car) => car.y < bestCarY + window.innerHeight
      );
